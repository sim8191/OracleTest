...
-- 프로시져에서 매개변수 사용
-- IN : 프로시져 실행시 필요한 값을 변수로, 일반적인 매개변수와 동일한 역활을 수행.
-- OUT : 호출한 곳으로 되돌려주는 변수(RETURN의 역활을 수행)
-- V_EMP_ID IN NUMBER
CREATE OR REPLACE PROCEDURE PRO_EMP(V_EMP_ID IN EMPLOYEE.EMP_ID%TYPE,
...
/*
    ...
    2. DB자원을 직접 사용하기 때문에 DB에 좀더 많은 자원을 주게 된다.
*/
-------------------------------------------------
/*
    <FUNCTION>
...
CREATE OR REPLACE FUNCTION CALC_SALARY(V_EMP_ID NUMBER) -- 뻑남 나중에 해보기
RETURN NUMBER
IS 
    E EMPLOYEE%ROWTYPE;
    RESULT NUMBER;
