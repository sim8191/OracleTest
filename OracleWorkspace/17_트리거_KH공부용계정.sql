/* -- 되도록 안쓰는 것을 권장
    <트리거>
     ...
     - EX) 회원탈퇴시 기존의 회원테이블에서 데이터를 삭제(DELETE)후, 곧바로 탈퇴된 회원들만
       보관하는 테이블에 자동으로 INSERT해야되는 경우.
       신고횟수가 일정수를 넘었을때 해당 회원의 정보를 즉시 변경해는 경우
       
     * 트리거 종류
     SQL문의 실행시기에 따른 분류
     1. BEFORE TRIGGER
      - 내가 트리거를 지정한 테이블에 DML이 발생되기 "전"에 트리거를 먼저 실행
     2. AFTER TRIGGER
      - 내가 트리거로 지정한 테이블에 DML이 발생되기 "후"에 트리거를 실행
      
    SQL문에 의해 영향을 받는 각 행에 따른 분류
    1. STATEMENT TRIGGER
     - 이벤트(DML)가 발생한 SQL문에 대해 딱 한번만 실행되는 트리거
    2. ROW TRIGGER
     - SQL문 실행시마다 매번 실행되게하는 트리거(FOR EACH ROW옵션 기술시 ROW TRIGGER로 설정)
...
*/
-- EMPLOYEE테이블에 새로운 행이 추가될때만다 자동으로 메세지를 출력해주는 트리거.
...
VALUES(777,'경민','123456-1234567','경민@NAVER.COM','J1','S1');
...
/*
    트리거 장점
    1. 데이터 추가, 수정, 삭제시 자동으로 데이터관리를 해줌으로써 데이터베이스의 무결성보장.
    2. 데이터베이스의 관리 자동화
    
    트리거 단점
    1. 빈번한 추가, 수정, 삭제시 다른 테이블의 행또한 추가로 수정, 추가, 삭제가 되므로 성능이
        좋지 못하다.
    2. 관리적 측면에서 형상관리가 불가능하다.
    3. 트리거를 남용하게되는 경우 유지보수가 힘들다.
*/
