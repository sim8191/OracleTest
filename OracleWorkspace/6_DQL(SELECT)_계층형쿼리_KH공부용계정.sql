/*
    ...
    1. 계층정인 데이터
     ...
     - 계층형 데이터에는 상위요소와 하위요소가 있으며 1개의 상위요소는 N개의 하위요소를
       포함하고 있다.
     - 계층형 데이터 예시 : 게임티어, 댓글, 카테고리, 조직도
    2. 계층형구조의 데이터 관리방법
    1) 여러 테이블에 데이터를 따로 관리
     - 상위요소, 하위요소별로 데이터를 별개의 테이블에 분활시켜 저장
     - 데이터의 중복을 최소화하고, 유연하게 설계할 수 있다.
     - 단, 계층구조가 복잡한 경우 사용하지 않는다.
    
    2) 하나의 테이블에서 관리
     - 하나의 테이블에 상위요소의 값과 하위요소의 값이 함께 저장되어 있는 구조
     - EX) EMPLOYEE테이블의 MANGER_ID는 상위요소, EMP_ID는 하위요소.
...
SELECT
    LEVEL,
--    LPAD(USER_NAME, LENGTHB(USER_NAME), ' ') 작성자,
...
-- 민경민이 작성한 최상위 댓글과 그 대댓글만 조회.
SELECT
    LEVEL, LPAD(' ',(LEVEL -1)* 2) || USER_NAME 작성자,
    CONTENT, CREATED_AT
    FROM COMMENTS
--    WHERE USER_NAME = '민경민' -- 자식요소가 안나옴
    START WITH PARENT_ID IS NULL 
    AND USER_NAME = '민경민' -- WHERE USER_NAME = '민경민'이거 확인하고 싶으면 여줄 지우기
    CONNECT BY PRIOR COMMENT_ID = PARENT_ID;

-- EMPLOYEE테이블에서 선동일 사원과 선동일 사원이 사수인 사원들의 조회
-- LEVER, 사번, 사원이름, 매니저 ID
SELECT LEVEL, EMP_ID ,  LPAD(' ',(LEVEL -1)* 2) || EMP_NAME , MANAGER_ID
FROM EMPLOYEE
