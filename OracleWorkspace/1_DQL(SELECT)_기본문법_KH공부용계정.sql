...
-- 대소문자 구분할필요는 없으나 대문자가 관례

select emp_id, EMP_name, salary
from employee; -- 대문자로 작성하는것이 권장된다.

-- EMPLOYEE테이블의 모든 행의 모든 칼럼을 조회
SELECT * -- *은 비권장
FROM EMPLOYEE; 

-- EMPLOYEE테이블의 전체 사원들의 이름, 이메일, 휴대폰 번호 조회
...
-- 3. DEPARTMENT테이블의 모든 칼럼 조화
SELECT * FROM DEPARTMENT;

-- 4. EMPLOYEE테이블이 직원명, 이메일, 전화번호, 입사일 칼럼 조회
...
/*
    <칼럼값을 통한 산술연산>
    - 조회하고자 하는 칼럼들을 나열하는 SELECT절에 산술연산자를 기술한
      결과를 조회
*/
-- EMPLOYEE테이블에서 직원명, 월급, 연봉(월급*12)
SELECT EMP_NAME, SALARY, SALARY * 12
FROM EMPLOYEE;

-- EMPLOYEE테이블로부터 직원명, 월급, 보너스, 보너스 포함 연봉을 조회
SELECT EMP_NAME, SALARY, BONUS, (SALARY + SALARY * BONUS) * 12
FROM EMPLOYEE;
-- 산술연산과정에서 NULL값이 존재하는 경우 산술연산의 결과마저도 NULL이 된다.

-- EMPLOYEE테이블에서 직원명, 입사일, 근무일수(오늘날짜 - 입사일) 조회
...
/*
    ...
    부여하고자하는 별칭에 특수문자난 띄어쓰기가 들어간 경우 ""로 묶어서 표기해야한다.
*/
-- 연봉 별칭부여
...
/*
    ...
    임의로 지정한 문자열 및 정수를 SELECT절에 기술하면 실제 그 테이블에 값이 존재하는것처럼
    조회가 가능하다.
*/
...
/*
    <DISTINCT>
     - 조회하고자 하는 컬럼에서 중복된 값을 제거하고 한번만 조회하고자 할 때 사용
     - SELECT절에는 1개의 DISTINCT구문만 사용할 수 있다.
*/
-- EMPLOYEE에서 부서코드들 조회
...
-- DEPT_CODE, JOB_CODE값으 세트로 묶어서 중복값인지 판별한다.(행기준 중복여부)
SELECT DISTINCT DEPT_CODE, JOB_CODE
FROM EMPLOYEE;
-- DEPT_CODE, JOB_CODE 이2개를 쌍으로 묶어서 중복을 제거(위에쓴 글이랑 같은거임)

-----------------------------------------------------------------
/*
    <WHERE절>
    - 조회하고자 하는 테이블에 특정 조건을 제시해서 그 조건에 맞는 데이터들만 조회할 수 있다.
    [표현법]
    SELECT 조회할 컬럼명...
    FROM 테이블명
    WHERE 조건식;
    ...
    실행순서
    FROM -> WHERE -> SELECT
    - 조건식에슨 다양한 연산자를 사용할 수 있다.
    
    <비교연산자>
    >, <, >=, <=
    =, !=, ^=, <> : 모두다 일치여부 검사하는거임
    (오라클애서는 == 이없고 =가 일치여부를 검사)
*/
-- EMPLOYEE테이블로부터 급여가 400만원 이상인 사원만 조회(모든칼럼)
SELECT *
FROM EMPLOYEE
WHERE SALARY>=4000000;

-- EMPLOYEE테이브로부터 부서코드가 D9인 사원들의 사원명, 부서코드 조회
...
--WHERE (SALARY*12)>=50000000;
-- 연봉은 실행순서상 WHERE절이 실행된 이후에 부여된 별칭이므로 사용 불가.
...
/*
    ...
    OR : 자바의 ||와 동일. ~이거나, 또는
*/
-- EMPLOYEE테이블에서 부서코드가 D9이면서 급여가 500만원 이상인 사원들의 이름, 부서코드
-- 급여를 조회.
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE dept_code = 'D9' AND salary >= 5000000;

-- 급여가 350만원 이상이고, 600만원 이하인 사원 들의 이름, 사번, 급여, 직급코드를 조회
...
-- NOT은 자바의 논리부정연산자와 동일한 역활의 연산자.

-- ORACLE에서는 DATE형식간의 범위에서도 BETWEEN AND 연산자와 대소비교연산자르 사용할 수 있다.
-- 입사일이 90/1/1 ~ 03/1/1인 사원들의 모든 칼럼 조회
SELECT *
FROM EMPLOYEE
...
/*
    ...
    '%' : 0글자 이상
        EX) 칼럼 LIKE '문자%' => 문자로 시작하는 칼럼을 조회
                     ...
                     '%문자%' => 문자가 포함된 칼럼을 조회
    '_' : 1글자
        EX) 칼럼 LIKE '_문자' => 3글자이면서 문자로 끝나는 칼럼을 조회
                     '__문자' => 4글자이변서 문자로 끝나는 칼럼을 조회
*/
...
-- 전화번호 4번째자리가 9로 시작하는 사원들의 사번, 사원명, 전화번호를 출력
SELECT EMP_ID, EMP_NAME, PHONE
FROM EMPLOYEE
WHERE PHONE LIKE '___9%';

-- 이메일중 네번째 문자위치에 _가 있는 사원의 모든정보
SELECT *
FROM EMPLOYEE
WHERE EMAIL LIKE '___\_%' ESCAPE '\';
-- 문자_를 검사에 사용하고 싶다면 이스케이핑 처리를 해야 한다.
-- ESCAPE처리를 수행할 문자를 지정하고, _앞에사용한다.
...
/*
    <IS NULL> = 시험에 나옴
     - 해당 값이 NULL인지 비교하는 NULL전용 비교연산자
     
    [표현법]
    비교대상칼럼 IS NULL => 칼럼이 NULL인 경우 참
    비교대상칼럼 IS NOT NULL => 칼럼값이 NULL이 아닌 경우 참
*/

-- 보너스를 받지 않는 사원의 사번, 이름, 급여, 보너스 조회
SELECT EMP_ID, EMP_NAME, SALARY, BONUS
FROM EMPLOYEE
WHERE BONUS IS NOT NULL; -- 보너스
--WHERE BONUS IS NULL;
...
SELECT EMP_ID || '번' || EMP_NAME || '의 월급은' || SALARY
|| '원 입니다.' AS "연결완료"
FROM EMPLOYEE;
...
/*
    ...
    FROM 테이블명
    WHERE 조건식
    ORDER BY [칼럼명/별칭/칼럼순번] [ASC/DESC] [NULLS FIRST/NULLS LAST]
    ASC : 오름차순, DESC: 내림
*/
...
SELECT *
FROM employee
ORDER BY salary; -- 생략시 ASC가 기본값
--ORDER BY salary ASC;
--ORDER BY salary DESC;

-- 보너스 기준 정렬
-- * ORACLE에서는 정렬시 NULL값을 가장 큰값으로 생략한다.
SELECT *
FROM employee
