/*
    ...
    SQL문 내에서 변수선언, 조건처리(IF), 반복척리(FOR), 예외처리등을 지원한다.
...
END;
/
-- 위의 /는 아무것도 해서는 안됨 건들지마

/*
    1. DECLARE 선언부
       변수 및 상수 선언 공간
       일반타입 변수, 레터런스 변수, ROW타입변수가 존재
...
BEGIN -- 뻑남(해결은 함 그래도 보셈)
    SELECT EMP_ID, EMP_NAME, job_code, SALARY * 12, dept_title
...
/*
    3) ROW타입 변수 선언
    테이블의 한 행에 대한 모든 칼럼값을 담을 수 있는 변수
    변수명 테이블명*ROWTYPE;
*/
DECLARE
    E EMPLOYEE%ROWTYPE;
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('보너스 : ' || E.BONUS); -- 보너스가 없는 경우 빈칸으로 표시
END;
/
----------------------------------------
/*
...
-- 중첩 반복문(NESTED LOOP)
-- EMPLOYEE테이블과 DEPARTMENT테이블을 JOIN이 아닌 중첩반복문을 활용하여 연결
BEGIN
...
BEGIN -- 뻑남
    ...
    IF EMP.BONUS IS NULL
        THEN SAL := EMP.SALARY * 12; -- 보너스 없는 경우
    ELSE SAL := (EMP.SALARY + EMP.SALARY * EMP.BONUS) *12; -- 보너스 있는 경우
    END IF;
    DBMS_OUTPUT.PUT_LINE(EMP.SALARY || '의 연봉은 ' || SAL || '입니다.');
END;
/
/*
...
/*
    ...
    * 시스템예외
     - NO_DATA_FOUND : SELECT 한 결과가 한행도 없는 경우
     - TO_MANY_ROWS : SELECT 한 결과가 여러행인 경우
...
BEGIN
    ...
    WHERE EMP_NAME = '노옹철';
EXCEPTION
    WHEN DUP_VAL_ON_INDEX THEN DBMS_OUTPUT.PUT_LINE('중복된 사번임');
END;
/
