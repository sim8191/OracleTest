...
-- CNAME칼럼을 새로 추가.
-- 칼럼추가 (ADD) : ADD 칼럼명 자료형 [DEFAULT 기본값]
...
ALTER TABLE DEPT_COPY MODIFY DEPT_ID NUMBER; -- CHAR(3) -> NUMBER? 안됨
...
SELECT * FROM DEPT_COPY; -- 직접 TABLE가서 보셈
...
/*
    ...
    - PRIMART KEY/FOREIGE KEY/UNIQUE/CHECK
      ADD 제약조건(칼럼명/조건식)
*/
-- DEPT_COPY테이블의 DEPT_ID에 PRIMARY KEY 제약조건 추가
-- DEPT_TITLE에 UNIQUE 제약조건, LNAME칼럼에 NOT NULL제약조건 추가.
ALTER TABLE DEPT_COPY
...
-- 칼럼명 변경 : RENAME COLUMN 기존칼럼명 TO 바꿀칼럼명;
ALTER TABLE DEPT_COPY RENAME COLUMN DEPT_TITLE TO DEPT_NAME;

-- 제약조건명 변경 : RENAME CONSTRAINT A TO B
-- SYS_C007193 -> DCOPY_DN_UQ (DEPT_COPY에서 확인하시요)
ALTER TABLE DEPT_COPY RENAME CONSTRAINT SYS_C007193 TO DCOPY_CN_UQ;

-- 테이블명 변경 : ALTER TABLE 테이블명 RENAME TO 바꿀테이블명
ALTER TABLE DEPT_COPY RENAME TO DEPT_TEST;
--ALTER TABLE DEPT_COPY TO DEPT_TEST; -- (현재버전에서는 안됨(그냥 안되는 문법일수도...))
...
-- 부모테이블 삭제하기
-- ALTER을 이용하여 부모-자식 관계 만들기
-- DEPT_TEST(부모), EMPLOYEE _COPY(자식)
...
-- 어딘가에서 참조되어 있는 부모테이블은 바로 삭제되지 않는다.
-- 이 부모테이블을 삭제하고 싶다면?
