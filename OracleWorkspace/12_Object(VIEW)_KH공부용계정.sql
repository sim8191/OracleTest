...
/*
    ...
    AS 서브쿼리;
*/
-- '한국'에서 근무하는 사원들의 사번, 이름, 부서명, 급여, 극무국가명, 직급명 조회
...
SELECT * FROM VW_EMPLOYEE
WHERE BONUS IS NULL; -- VIEW에 존재하지 않는 칼럼사용시 오류발생

CREATE OR REPLACE VIEW VW_EMPLOYEE -- OR REPLACE 그 안에 있는 내용 변경(막쓸수 있는 거는 아님 뷰에서는 사용가능?)
...
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE); -- 칼럼에 함수를 사용한 경우 컬럼부여가 필수
...
/*
    VIEW를 활용한 DML(INSERT, UPDATE, DELETE)
     - 뷰를 통해서 DML을 수행하는 경우, 실제 데이터를 저장하고 있는 테이블에 변경사항이
     적용된다.
*/
...
UPDATE VW_JOB
SET JOB_NAME='알바'
WHERE JOB_CODE = 'J8'; -- 뻑남

/*
    * DML이 가능한 경우 : 서브쿼리를 이용한 결과값이 기존의 테이블과 별도의 차이가 없는 경우
    
    * 뷰를 통해 DML이 불가능한 경우
    1) 뷰에 정의되지 않은 칼럼을 조작하는 경우
     2)뷰에 정의되어 있지 않는 칼럼 중 NOT NULL제약조건이 추가된 칼럼이 존재하는 경우
    3) 산술연산식 또는 함수를 통한 칼럼이 정의되어 있는 경우
    4) 크룹함수나 GROOUP BY절이 포함된 경우
    5) DISTINCT구문이 포함된 경우
    6) JOIN을 통해 여러 테이블을 매칭시켜놓은 경우
*/
--1) 뷰에 정의되지 않는 칼럼을 조작
--2) 뷰에 정의되어 있지 않은 컬럼 중, 원본테이블상에 NOT NULL제약조건이 추가된 경우.
CREATE OR REPLACE VIEW VW_JOB
...
--3) 산술연산식 또는 함수를 통해서 정의된 칼럼이 존재하는 경우
CREATE OR REPLACE VIEW VW_EMP_SAL
AS SELECT EMP_ID, EMP_NAME, SALARY, SALARY * 12 연봉
FROM EMPLOYEE;

-- virtual column not allowed here오류 발생
INSERT INTO VW_EMP_SAL VALUES(400, '경민', 3000000, 36000000);
-- 가상컬럼은 값을 변경할 수 없음(예시 SALARY * 12 연봉)
...
AS SELECT EMP_ID, EMP_NAME, DEPT_CODEF
FROM EMPLOYEE
LEFT JOIN department NO DEPT_ID = DEPT_CODE; -- 뻑남
...
UPDATE VW_JOINEMP
SET EMP_NAME = '서동이'
WHERE EMP_ID = 200; -- 하나의 테이블에서 조작이 발생하는 경우 변경 성공

UPDATE VW_JOINEMP
SET DEPT_TITLE = '회계부'
WHERE EMP_ID = 200; -- 여기는 버전에따라서 가능할수도 있고 안할수도 있음
-- 위에서 뻑나서 여기까지 안됨
ROLLBACK;
...
AS SELECT A,B,C FROM FORCETABLE;

SELECT * FROM V_FORCETEST; -- 위만 하면 오류남

CREATE TABLE FORCETABLE( -- 여기를 해줘야 오류가 안남
    A NUMBER,
    B NUMBER,
    C NUMBER
);
...
-- 3. WITH CHECK OPTION
-- SELECT 문의 WHERE절에서 사용한 칼럼을 수정하지 못하게 막는 옵션.
CREATE OR REPLACE VIEW V_CHECYOPION
...
UPDATE V_CHECYOPION
SET SALARY = 6000000
WHERE EMP_ID = 206; -- WHERE절에 기술한 칼럼이 아니므로 변경 가능.
