/* 
    <SUBQUERY>
     ...
     - 메인 SQL문을 보조하기 위해 사용
     - SELECT, FROM, WHERE 때 위치에 따라 다양하게 활용되며, 부르는 명칭이 다르다
...
/* 
    서브쿼리의 구분
     - 서브쿼리를 실행한 결과값이 몇행 몇열 이냐에 따라서 분류한다.
...
-- 부서별 급여 합이 가장 큰부서 하나만을 조회. 부서코드, 부서명, 급여의합을 출력
...
/* 
    2. 다중행 서브쿼리 (MULTIROW SUBQUERY)
     ...
     - IN (서브쿼리) : 여러개의 결과값 중 하나라도 일치하는 것이 있다면 참을 반환
     - 칼럼 [>/<] ANY (서브쿼리) : 여러개의 결과값 중 "하나라도" 크거나 작을 경우 참을 반환
...
-- 과장 직급임에도, 모든 차장직급의 급여보다 더 많이 받는 직우너 조회
SELECT EMP_ID , EMP_NAME, JOB_NAME, SALARY
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
...
-- 소속된 부서가 없는 경우 NULL처리함수를 통해 부서없음으로 조회하시오.
...
/* 
    5. 스칼라 서브쿼리 
     - 단일행 단일열 서브쿼리의 일종으로 SELECT절에서 사용되는 서브쿼리를 의미한다.
       SELECT절이 실행될때마다 서브쿼리문이 실행되면서 조회결과값을 반환한다.
     - 현재 조회된 행의 칼럼값을 서브쿼리내에서 기술이 가능하며, 기술하는 경우 연관쿼리라고
       부른다
     - 매행마다 실행되기 때문에 대규모 데이터 처리시 효율이 좋지 못하다(연관쿼리)
*/
...
FROM (
    SELECT ROWNUM AS R, EMP_NAME, SALARY
    FROM (
        SELECT * 
        FROM EMPLOYEE 
        ORDER BY SALARY DESC
