...
INSERT INTO MEMBER VALUES('user03','pass03','민경민', sysdate);
SELECT * FROM MEMBER;
...
/* 
    2. UNIQUE 제약조건
     - 칼럼에 중복값을 제한하는 제약조건
     - 칼럼에 값이 삽입/수정될 때 기존에 해당 칼럼값 중 중복값이 잇을 경우
       추가 또는 수정되지 않게 제약한다.
*/
...
(1, 'user01', 'pass01','기롱' , '남');

-- CHECK제약조건으로 NULL값 추가를 방지하고 싶다면, NOT NULL제약조건을 추가해야한다.
INSERT INTO MEM_CHECK VALUES
(2, 'user02', 'pass01','기롱' , NULL );
...
WHERE GRADE_CODE = 'G1';
-- 자식테이블에서 변경/삭제하고자하는 행을 참조하는 경우, 제대로된 변경/삭제가 불가능하다.
-- ORA-02292: integrity constraint (DDL.SYS_C007125) violated - child record found
...
/*  
    * FOREIGN KEY 삭제 옵션
     ...
     2. ON DELETE SET NULL : 부모데이터 삭제시 해당 데이터를 사용하는 자식데이터를
        NULL로 변환
     3. ON DELETE CASCADE : 부모데이터 삭제시 해당 데이터를 사용하고 있는 자식데이터
     를 함께 삭제하는 옵션.
*/
