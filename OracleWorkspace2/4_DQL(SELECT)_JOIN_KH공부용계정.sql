/* 
    ...
    - 관계형데이터베이스(RDBMS)에서는 데이터의 중복을 피하기 위해, 다양한 테이블에 데이터
    를 나눠서 보관하고 있으며, 이 때 여러테이블의 데이터를 동시에 조회하기 위해서는 
    ...
    -------------------------------------------------------
        ...
        (RIGHT OUTER JOIN)      |  RIGHT OUTER JOIN
                                |  FULL OUTER JOIN
    --------------------------------------------------------
    카테시안의곱(CARTESIAN PRODUCT)| 교차조인(CROSS JOIN)
    --------------------------------------------------------
        자체조인(SELF JOIN)
...
/* 
    3. 카테시안의 곱 / 교차조인
     ...
     - 즉, 두 테이블의 행든이 모두 곱해진 행들의 조합이 출력
*/
-- 사원명, 부서명
...
SELECT EMP_NAME, SALARY, S.SAL_LEVEL 
FROM EMPLOYEE 
JOIN SAL_GRADE S ON (SALARY BETWEEN MIN_SAL AND MAX_SAL) AND 1 = 1;

/* 
    5. SELF JOIN(자체조인)
    - 같은 테이블끼리의 조인 ( A JOIN A)
    - 자체조인을 사용할 경우 각 테이블에 "반드시" 별칭을 부여해야 한다.
    - 계층적인 구조의 데이터를 다룰때 주로 사용된다. 
