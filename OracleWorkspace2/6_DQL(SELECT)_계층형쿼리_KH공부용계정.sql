/* 
    ...
    1) 여러 테이블에 데이터를 따로 관리
     - 상위요소, 하위요소별로 데이터를 별개의 테이블에 분한시켜 저장
...
SELECT
    ...
    COMMENT_ID , PARENT_ID, CONTENT, CREATED_AT
    FROM COMMENTS
    -- 계층구조의 시작점을 정의
...
CONNECT BY PRIOR COMMENT_ID = PARENT_ID
;
-- EMPLOYEE테이블에서 선동일 사원ㅇ과 선동일 사원이 사수인 사원들으 조회
-- LEVEL, 사번, 사원이름, 매니저 ID
SELECT
    LEVEL, EMP_ID, LPAD(' ', (LEVEL-1)*2 ) || EMP_NAME, MANAGER_ID
FROM EMPLOYEE
