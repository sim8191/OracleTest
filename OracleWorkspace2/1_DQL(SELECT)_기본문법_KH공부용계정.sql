...
-- EMPLOYEE에서 직원명, 입사일, 근무일수(오늘날짜 - 입사일) 조회
...
-- DEPT_CODE와 JOB_cODE값을 세트로 묶어서 중복값인지 판별한다.(행기준 중복여부)
...
FROM EMPLOYEE
WHERE QUIT_YN = 'N';
-- 4. EMPLOYEE 테이블에서 연봉 (급여 * 12) 이 5000 만원 이상인 사원들의 
-- 이름, 급여, 연봉, 입사일 조회
...
-- 급여가 350만원 이상이고, 600만원 이하인 사원들의 이름, 사번, 급여, 직급코드를 조회
...
-- ORACLE에서는 DATE형식간의 범위에서도 BETWEEN AND 연산자와 대소비교연산자를 사용할 수 있다.
    ...
    <LIKE '패턴'>
     ...
     '%' : 0글자 이상
           EX) 칼럼 LIKE '문자%' => 문자로 시작하는 칼럼을 조회
                         ...
                         '%문자%' => 문자가 포함된 칼럼을 조회
     '_' : 1글자
           EX) 칼럼 LIKE '_문자' => 3글자이면서 문자로 끝나는 칼럼
                         '__문자' => 4글자이면서...
*/
...
-- ESACPE처리를 수행할 문자를 지정하고, _앞에 사용한다. 
...
/*
    <ORDER BY 절>
     ...
     FROM 테이블명
     WHERE 조건식
     ORDER BY [컬럼명/별칭/칼럼순번] [ASC/DESC] [NULLS FIRST/NULLS LAST]  
*/

-- 월급이 높은사람부터 나열하고 싶다(월급기준 내림차순)
SELECT * 
FROM EMPLOYEE
ORDER BY SALARY DESC; -- 생략시 ASC가 기본값

-- 보너스 기준 정렬
--  * ORACLE에서는 정렬시 NULL값을 가장 큰값으로 생각한다.
SELECT *
FROM EMPLOYEE
