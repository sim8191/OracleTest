/* 
    <함수 FUNCTION>
     ...
     - 그룹함수 : 
            N개의 값을 읽어서 M개(기본값 1)의 결과를 리턴하는 함수(그룹별 함수 실행후 결과 반환)
...
/* 
    LTRIM/RTRIM
     ...
     - 문자열기준 왼쪽/오른쪽에서부터 제거시키고자 하는 문자들을 찾아서 제거한 나머지를 반환
     - 제거할 문자 생략시 기본값은 ' '.
*/
SELECT LTRIM('     K   H    ')
FROM DUAL; -- K   H    
...
SELECT LTRIM('1313KH123','1234567890')
FROM DUAL;
...
/*
    ROUND
     - ROUND(숫자, 반올림할 위치)
     - 반올림할 위치 : 소숫점 기준 N번째수에서 반올림 처리해주며 생략가능.
                     생략시 소숫점을 모두 없애버린다.
*/
...
/* 
    ...
    TO_CHAR(NUMBER/DATE , 포맷)
     - 숫자형 똔느 날짜형 데이터를 문자형 타입으로 반환(포맷에 맞춰서) 
*/
...
-- 
SELECT TO_NUMBER('10,000','99,999') + TO_NUMBER('50,000','99,999')
FROM DUAL;
...
/* 
    ...
    - NVL은 컬럼에 값이 존재하는 경우 컬럼값을, 존재하지 않는경우 대체값을 반환
*/
--사원명, 보너스, 보너스가 없는경우 0으로출력
...
/* 
    <선택함수>
    - DECODE(칼럼, 값1, 결과값1, 값2, 결과값2, 값3, 결과값3 , .... , 결과값)
    - 자바의 SWITCH문과 유사
    switch(비교대상칼럼) {
    case 값1 -> 결과값1; 
    case 값2 -> 결과값2; 
...
/*
    CASE WHEN THEN 구문 
     - DECODE는 조건검사시 동등비교만을 수행한다면, CASE WHEN THEN구문은 내마음대로
     조건을 설정할 수 있다. 
...
SELECT AVG(SALARY)
FROM EMPLOYEE;
--WHERE 1 = 0; -- 조건에 맞는 값이 없는 경우 기본값 NULL
...
-- 5. MIN(ANY칼럼)
-- 정수기준 가장 작은값, 문자기준으로는 사전등록순서상 가장 낮은값, 날짜기준으로는 가장 과거값
...
/*
    ...
    DENSE_RANK() OVER(그룹화기준 정렬기준) 
     - 공동 1위가 3명이라고 해도, 그 다음순위는 무조건 2위로 처리하는 방식
    ROW_NUMBER() OVER(그룹화기준 정렬기준) 
     - 결괎에대해 고유한 번호를 부여하는 함수로, 공동순위가 존재하지 않는다. 

    OVER([PARTITION BY 칼럼] ORDER BY 칼럼) 
     - PARTITON : 데이터를 분할하는 행위를 의미하며, 칼럼기준으로 데이터를 분할할 수 있다.
...
/*
    ...
    2) LEAD 
     - 나아가다
     - 현재형 기준 뒤의 값을 가져올때 사용
     - LEAD(가져올칼럼, 행번호, 기본값) OVER(분할기준,정렬기준)
*/
...
SELECT EMP_NAME, DEPT_CODE, SALARY, LAG(SALARY, 2 , 0) OVER(ORDER BY SALARY DESC) "이전직원월급"
FROM EMPLOYEE;

-- 실습 -- 
-- 직원의 사원번호, 이름, 직급코드, 월급을 조회한 후
-- 조회결과내에서 직급코드별로 그룹화 하여, 소그룹내에서 전사람의 월급을 가져오시오
-- 단, 가져올 데이터가 없는 경우, 0원으로 표시하며 소그룹내에서는 사원번호 기준 오름차순
--  정렬하시오.
