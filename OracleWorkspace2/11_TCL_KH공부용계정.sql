/*
    TCL(TRANSACTION CONTROL LANGUAGE) 
     ...
     - COMMIT : 하나의 트랜잭션에 담겨있는 변경사항들을 실제 DB에 반영하는 명령어
                COMMIT실행후, 트랜잭션 내부의 변경사항은 모두 비워진다(CLEAN)
...
/*
    ...
    1. Atomicity(원자성)
     ...
     - 즉, 트랜잭션 내부작업을 commit,rollback으로 상세하게 쪼개어 처리할 수 없기
     때문에 원자성의 특징을 가진다.
*/
-- EMP_01테이블생성
CREATE TABLE EMP_01
...
SELECT * FROM KH.EMP_01; -- 관리자계정 (26명)
...
-- 준수하지 않은 데이터를 추가.
ALTER TABLE EMP_02 ADD PRIMARY KEY(EMP_ID);

-- 기본키 제약조건 위배. 
-- 데이터베이스의 일관성 유지를 위해서 아래 DML문은 트랜잭션에 추가되지 않는다.
INSERT INTO EMP_02 VALUES(200, '선동일2','총무부');

INSERT INTO EMP_02 VALUES(800, '선동일2','총무부');
COMMIT;
/*
    3. Isolation(격리성) 
     - 서로 다른 세션에서 동시에 여러 트랜잭션이 실행되는 경우, 각 트랜잭셩이 서로의 작업에
     ...
     는 트랜잭션의 DML이 완전히 COMMIT,ROLLBACK되기 전까지는 다른 세션에서 DML을 수행하지
     못하도록 LOCK을 건다.
